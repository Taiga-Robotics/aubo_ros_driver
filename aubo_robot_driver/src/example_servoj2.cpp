...
int exampleServoj3(RpcClientPtr cli)
{
    ...
    while(do_loop)
    {
        ...
        q[5] += (M_PI/2)*sin(elapsed_seconds.count()*2.); 
        std::cout << "Stream q: " << q << std::endl;
        robot_interface->getMotionControl()->servoJoint(q, 10., 1.0, 0.05, 200, 200);
